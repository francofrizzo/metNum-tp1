function hornoVsPosta(inputfile, solution, isofile, solout, isoout, inputposta, solposta, isoposta)
...
for k = 1:ninst
    ...
    if (nargin > 2)
        ...
        if (nargin > 7)
            ipid = fopen(inputposta);
            solpid = fopen(solposta);
            postaparam = fscanf(ipid, '%d',6);
            isopid = fopen(isoposta);

            angulosposta = postaparam(4);
            radiosposta = postaparam(3);
            radioiposta = postaparam(1);
            radioeposta = postaparam(2);
            puntosposta = angulos*radios;
            calorisoposta = postaparam(5);
            ninstposta = postaparam(6);
            deltaAngposta = 2*pi/angulos;
            deltaRadposta = (radioe-radioi)/(radios-1);

            radioisoposta = fscanf(isopid, '%f',angulosposta);
            radioisoposta = [radioisoposta; radioisoposta(1)];
            radioisoposta = radioisoposta';

            thetaposta=linspace(0,2*pi,angulosposta+1);
            internoposta=linspace(radioiposta,radioiposta,angulosposta+1);
            externoposta=linspace(radioeposta,radioeposta,angulosposta+1);
            polar(thetaposta,externoposta, 'r'), hold on;
            polar(thetaposta,radioisoposta, 'r');
            polar(thetaposta,internoposta, 'r');
        end
