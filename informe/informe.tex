...
\section{Desarrollo}
  ...
  \begin{proof}
    ...
    \begin{itemize}
      \item[\textbf{C.B.}] Teniendo en cuenta que $A_{1, 1} \neq 0$ (ver Observación \ref{obs:Diagonal de A sin ceros}), se deduce trivialmente que es posible aplicar el primer paso de Eliminación Gaussiana sin realizar pivoteo. Del Lema \ref{lema:EG conserva diagonal dominante}, se sigue que la matriz $A^{(1)}$ obtenida será diagonal dominante. Como $A_{1, 2} = 0$, tendremos $A^{(1)}_{2,2} = A_{2,2}$, que es necesariamente no nulo (Observación \ref{obs:Diagonal de A sin ceros}), por lo que la segunda fila de $A^{(1)}$ será no nula.
